...
wnba_gl <- wnba_gl |>
  mutate(
    ...
    loss_aversion = elo_home_win_prob - 0.5
  )
```
...
plot_attend <- ggplot(wnba_gl, aes(x = elo_home_win_prob, y = attendance, color = factor(is_cc))) +
  geom_point() +
...
f_exp <- c("I(quality_of_play)", "I(uncertain_outcome)", "I(loss_aversion)")
...
mod_attend_coefs |>
  ...
  mutate(
    ...
    scaled_estimate = case_when(
      model == "f1" ~ estimate * (mean(wnba_gl$quality_of_play)),
      model == "f2" ~ estimate * (mean(wnba_gl$uncertain_outcome)),
      model == "f3" ~ estimate * (mean(wnba_gl$loss_aversion))
    )
  ) |>
